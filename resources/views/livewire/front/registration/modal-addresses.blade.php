<div class="fixed z-50 flex top-10 md:top-32">
    <x-modal.dialog wire:model='formModalAddressOpened' title='Endereço de Entrega do Cliente:' class="">
        <div class="w-full space-y-3 ">
            ...
            <div class="flex items-baseline space-x-2 space-y-3">
                <x-form.label for="adShip.type" value="{{ __('Tipo') }}" />
                <select wire:model.defer="adShip.type" class="rounded-lg -py-1">
            ...
            <div class="flex items-baseline space-x-2 space-y-3 md:space-x-6 md:space-y-0">
                ...
                <div class="flex items-baseline space-x-2 space-y-3">
                    ...
                    <x-form.text wire:model.defer="adShip.postcode" id="adShip-postcode" type="text"
                        class=" w-[95px] text-center mt-1" />
                    <x-button.secundary wire:click="searchAddress" class="w-1/2 text-center h-7">Importar Rua
                    </x-button.secundary>
                    ...
                    <div class="flex items-baseline space-x-2 space-y-3 md:space-x-6 md:space-y-0">
                        ...
                        <x-form.text wire:model.defer="adShip.street" id="adShip-street" type="text"
                            class="mt-1 w-80" />
                    ...
                    <div class="flex items-baseline space-x-2 space-y-3 md:space-x-2 md:space-y-0">
                        <x-form.label for="adShip.number" value="{{ __('Nº') }}" />
                        <x-form.text wire:model.defer="adShip.number" id="adShip-number" type="text"
                            class="w-[50px] mt-1" />
            ...
            <div class="flex flex-col space-y-3 md:flex-row md:space-x-6 md:space-y-0">
                ...
                <div class="flex items-baseline space-x-2 space-y-3 md:space-x-2 md:space-y-0">
                    ...
                    <x-form.text wire:model.defer="adShip.neighborhood" id="adShip-neighborhood" type="text"
                        class="w-full mt-1" />
                    <x-alert.error-form for="adShip.neighborhood" class="mt-2" />
                </div>


                <div class="flex items-baseline space-x-2 space-y-0 md:space-x-2 md:space-y-0 ">
                    <x-form.label for="cityName" value="{{ __('Cidade') }}" />
                    <x-form.text wire:model.defer="cityName" rows="3" id="cityName" class="w-[150px] mt-1"
                        readonly />
                ...
                <div class="flex items-baseline space-x-2 space-y-3 md:space-x-2 md:space-y-0">
                    ...
                    <x-form.text wire:model.defer="cityState" rows="3" id="cityState" class="w-10 mt-1 "
                        readonly />
            ...
            <div class="flex items-baseline space-x-2 space-y-3 md:space-x-6 md:space-y-0">
                ...
                <x-form.text wire:model.defer="adShip.complement" rows="3" id="adShip-complement"
                    class="w-full mt-1" />
